...
Mr = matrix(CDF,[[0,-9,0,0,0,3,7.2,0,0,0,0],[-9,0,0,0,-3,0,0,0,0,0,-8.8],[0,0,0,-4.4-5*I,1.3-2*I,0,6.2,0,0,-4.4,0],[0,0,-4.4+5*I,0,3.5,-9.4,0,0,3.2,0,0],[0,-3,1.3+2*I,3.5,0,0,0,0,0,0,6.1],[3,0,0,-9.4,0,0,0,3.7,-9.9,0,0],[7.2,0,6.2,0,0,0,0,-6.5,0,-1.2,0],[0,0,0,0,0,3.7,-6.5,0,-9.3,5.4,8.2],[0,0,0,3.2,0,-9.9,0,-9.3,0,-2.9,-5.7],[0,0,-4.4,0,0,0,-1.2,5.4,-2.9,0,7.6],[0,-8.8,0,0,6.1,0,0,8.2,-5.7,7.6,0]])
print Mr.is_hermitian()
print [real(x) for x in Mr.eigenvalues()]
print "Inertia Bound: {}".format(min(sum(real(x)>=0 for x in Mr.eigenvalues()), sum(real(x)<=0 for x in Mr.eigenvalues())))
...
def ComplexSigning(G,m):
    ...
    for combo in Combinations(numVertices,2*alpha+1):
        J = G.subgraph(combo,immutable=true)
    ...
    for n in neg.keys():
        ...
        old = None
        tempNegCount = 0
        for edge in neg[n].edges():
            old = m[edge[0],edge[1]]
            m[edge[0],edge[1]] = old + (old+1)*I
            m[edge[1],edge[0]] = old - (old+1)*I
            tempNegCount = 0
            for s in subgraphs.keys():
                vertices = s.vertices()
                submatrix = m[vertices,vertices]
                if submatrix.determinant() < 0:
                    tempNegCount += 1
            edges[edge] = tempNegCount - negSubs
            m[edge[0],edge[1]] = old
            m[edge[1],edge[0]] = old
        counter += 1
        if counter == 4:
